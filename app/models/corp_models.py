...
class CorpUpgrades(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    industrial_production: int = Field(default=1) # Increase Production Amount
    industrial_level: int = Field(default=1)      # Unlocks better items to craft
    criminal_networks: int = Field(default=1)          # Increase success rate of crime
    criminal_money_laundering: int = Field(default=1)  # Increase chance to get away with capital
