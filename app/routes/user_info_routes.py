...
async def get_user_stats(session: Session = Depends(get_session), user: User = Depends(get_current_user)):
    try:
        session.add(user)
        if not user.stats:
        ...
        raise HTTPException(status_code=404, detail={"message": "Error getting inventory"})



@user_info_router.get("/get-user-inventory")
async def get_user_items(session: Session = Depends(get_session), user: User = Depends(get_current_user)):
    try:
        session.add(user)
