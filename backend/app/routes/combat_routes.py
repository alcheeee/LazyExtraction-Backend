...
async def build_weapon(
    ...
    weapon_handler = WeaponStatsHandler(session, user_id, request.weapon_inventory_id)
    weapon_item, updated_weapon = await weapon_handler.apply_attachments(request.attachments_to_add)
    effective_stats = weapon_handler.calculate_effective_stats(weapon_item.weapon_details, updated_weapon.modifications)
